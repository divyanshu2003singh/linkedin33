...
let MAX_PAGES = 3;
let CSV_FILE_PATH = 'poland_linkedin.csv';
...
(async () => {
    try {
        ...
        await page.type(inputFieldSelector, USERLOCATION);
        await page.waitForTimeout(1000);
        ...
        await page.type('.search-reusables__filter-value-item:nth-child(3) input[type="text"]', USERJOBTITLE);
        ...
        for (let currentPage = 1; currentPage <= 3; currentPage++) {
            ...
            const url = `https://www.linkedin.com/search/results/people/?geoUrn=${geoUrn}&keywords=${USERKEYWORDS}&origin=FACETED_SEARCH&page=${currentPage}&titleFreeText=${USERJOBTITLE}`;
        ...
        const csvWriter = createCsvWriter({
            path: 'linkedin_profiles.csv', // Use config.csvFilePath if available, otherwise default to 'linkedin_profiles.csv'
            header: [
                { id: 'name', title: 'Name' },
